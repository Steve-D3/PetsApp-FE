...
export const MedicationsSection = ({
  ...
  const renderMedicationItem = (medication: ViewAllMedication) => {
    const medItem = medication as unknown as Medication;
    return (
      <div 
        key={medication.id}
        className="p-3 hover:bg-gray-50 rounded-lg cursor-pointer"
        onClick={() => {
          setSelectedMedication(medItem);
          setIsDetailModalOpen(true);
      ...
      >
        <div className="flex items-start">
          <div className="bg-purple-50 p-2 rounded-lg mr-3">
            {medItem.icon || <Pill className="h-5 w-5 text-purple-500" />}
          </div>
          <div className="flex-1">
            <p className="font-medium text-gray-900">{medItem.name}</p>
            <div className="flex items-center mt-1 text-sm text-gray-500">
              <Calendar className="h-4 w-4 mr-1.5 flex-shrink-0" />
              <span>Start: {medItem.startDate || 'No start date'}</span>
              {medItem.endDate && (
                <span className="ml-2">• End: {medItem.endDate}</span>
              )}
            </div>
            <p className="text-sm text-gray-500 mt-1">
              {medItem.dosage} • {medItem.frequency}
            </p>
          </div>
        </div>
      </div>
